...
Direct successor to [sources/optimize-anything](optimize-anything.md). Where the original post argued *"anything text-representable with a measurable score can be optimized"* and shipped GEPA as the one optimizer behind that API, this update **decouples the API from the optimizer** and turns the choice of optimizer into a pluggable, composable dimension.

## One-line summary

`optimize_anything` becomes **engine-pluggable** (`engine="gepa" | "autoresearch" | "meta_harness"`) and **pipeline-composable** (chain / race / vote engines), and introduces **omni** — a meta-optimizer that runs a *portfolio* of engines in parallel and then continues the winner with a fresh optimizer. The empirical punchline: **no single optimizer dominates, but every omni composition beats every standalone optimizer.**

## Why this matters for the wiki

The three pluggable engines are **three systems this wiki already tracks separately**, now unified under one contract:
...
This is the first source in the wiki that treats *"which self-improvement architecture do I use"* as itself an optimizable/composable choice — a peer-level generalization of [sources/evox](evox.md)'s meta-evolution, but over **whole optimizer families** rather than over search strategies inside one family.
...
- **`info` dict = Actionable Side Information (ASI)** — the same [rich-feedback](../concepts/feedback-signals.md) channel from the original GEPA post; the proposer reads it to decide what to mutate.
...
This is a concrete, budgeted instantiation of the [sources/evox](evox.md) insight ("no single search strategy dominates across all tasks and phases") lifted one level up: no single *optimizer* dominates across tasks, so race them and continue the leader.
...
`optimize_adaptive_sequential` is the closest analog to [sources/evox](evox.md)'s stagnation-triggered strategy switching — but the thing being switched is the whole engine, not a sampling strategy.

## Terrarium (controlled evaluation harness)

To compare engines fairly, the team released **Terrarium**, which pins every optimizer to identical tasks, budgets, models (**Claude Sonnet 4.6, medium thinking**), and evaluation servers. This is the apparatus that makes the "no single optimizer dominates" claim credible rather than anecdotal — a shared-substrate benchmark for *optimizers*, analogous to how [sources/meta-harness](meta-harness.md) fixed a benchmark to compare harness edits.
...
2. **Every omni variant outscores every standalone optimizer.** Omni-AutoResearch is the best overall (63.2); omni-GEPA (61.8) alone narrowly beats *standalone* AutoResearch (55.4) — i.e., wrapping the weakest standalone engine in omni made it competitive with the strongest standalone.

The largest lift lands on GEPA (+41%), the engine most prone to early plateau — consistent with the "fresh optimizer breaks plateaus" thesis.

## What is optimized / what is not

**Optimized**: the target artifact (code, prompt, config — anything text-representable with a score), *and* — via omni — the sequence/portfolio of optimizers applied to it.

**Not optimized**: the base LLM weights (Sonnet 4.6 is a frozen operator throughout), and the omni schedule itself (the two-phase explore/continue split and the equal budget shares are fixed policy, not learned). In [concepts/harness-optimization](../concepts/harness-optimization.md) terms this is still a solution/harness-space optimizer; omni adds a fixed **meta-scheduling** layer over the optimizer choice.
...
- Unifies [sources/optimize-anything](optimize-anything.md) (GEPA), [sources/autoresearch-vs-hpo](autoresearch-vs-hpo.md) (AutoResearch), and [sources/meta-harness](meta-harness.md) (Meta-Harness) as interchangeable engines — a synthesis the wiki had described only as separate loop architectures.
- Peer to [sources/evox](evox.md): both say "no single X dominates, adapt/portfolio across X," at the strategy level (EvoX) vs. the whole-optimizer level (omni).
...
- [sources/evox](evox.md) — meta-evolution; omni is the same "adapt across searchers" idea one level up
- [concepts/evolutionary-optimization](../concepts/evolutionary-optimization.md) — portfolio/meta-optimizer sits at the top of the hierarchy of evolution
